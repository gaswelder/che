...
void GenSubtree(FILE *out, ObjDesc *od)
{
    ...
    switch(od->id) {
        ...
        case COUNTRY:
            ...
            xmlprintf(out, "%s", countries[GenContents_country]);
            break;
        case PROVINCE:
            if (GenContents_country==countries_USA)
                xmlprintf(out,"%s", provinces[ignuin(0,provinces_len-1)]);
            else
                xmlprintf(out,"%s", lastnames[ignuin(0,lastnames_len-1)]);
            break;
        case EDUCATION:            
        ...
        case HAPPINESS:
            ...
            break;
        case HOMEPAGE:
            xmlprintf(out,"http://www.%s/~%s", emails[GenContents_email],lastnames[GenContents_lstname]);
            break;
        case STREET:
            r=ignuin(0,lastnames_len-1);
            xmlprintf(out,"%d %s St",ignuin(1,100),lastnames[r]);
            break;
        case PHONE:
            int country=ignuin(1,99);
            int area=ignuin(10,999);
            int number=ignuin(123456,98765432);
            xmlprintf(out,"+%d (%d) %d",country,area,number);
            break;
        case CREDITCARD:
            for(int i=0;i<4;i++) {
                char *s = "";
        ...
        case EMAIL:
            ...
            xmlprintf(out,"mailto:%s@%s",lastnames[GenContents_lstname],emails[GenContents_email]);
            break;
        case GENDER:
            r=ignuin(0,1);
            if (r < 1) {
