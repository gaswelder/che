...
void shift(peer_t *p, size_t n) {
	if (n > p->stashlen) {
		panic("incorrect shift: have %zu, shifting %zu", p->stashlen, n);
	}
...
pub void handle(void *ctx, int event, void *edata) {
	...
	switch (event) {
		case ioloop.CONNECT_FAILED: { printf("connect failed, err=%s\n", strerror(errno)); }
		...
		case ioloop.DATA_IN: {
			...
			while (true) {
				...
				shift(state, nread);
				if (nwritten > 0) {
					if (!ioloop.write(ctx, (char *) tmpbuf2, nwritten)) {
						panic("write failed");
					}
				}
			}
		...
		default: { panic("peer: event=%d, data=%p\n", event, edata); }
	}
}
