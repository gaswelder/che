...
fn expect(l: &mut Lexer, kind: &str) -> Result<Token, Error> {
    if !l.more() {
        return Err(Error {
            message: format!("expected '{}', got end of file", kind),
            pos: l.pos(),
        });
    }
    let next = l.peek().unwrap();
    if next.kind != kind {
        return Err(Error {
            message: format!("expected '{}', got '{}'", kind, token_to_string(next)),
            pos: next.pos.clone(),
        });
    }
